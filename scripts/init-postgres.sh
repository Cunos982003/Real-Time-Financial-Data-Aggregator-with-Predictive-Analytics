...
PSQL_DB="${POSTGRES_DB:-financial_data}"
PSQL_USER="${POSTGRES_USER:-postgres}"

echo "Initializing PostgreSQL database on $PSQL_HOST:$PSQL_PORT..."

export PGPASSWORD="${POSTGRES_PASSWORD:-postgres}"

psql -h "$PSQL_HOST" -p "$PSQL_PORT" -U "$PSQL_USER" -d postgres -c "SELECT 1" > /dev/null 2>&1 || {
    echo "ERROR: Cannot connect to PostgreSQL"
    exit 1
}

psql -h "$PSQL_HOST" -p "$PSQL_PORT" -U "$PSQL_USER" -d postgres << 'EOF'
...
-- Features table (partitioned by symbol)
